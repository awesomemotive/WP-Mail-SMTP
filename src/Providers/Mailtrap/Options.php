	...
	public function __construct( $connection = null ) {
		...
		$description = sprintf(
			...
			/* translators: %1$s - URL to mailtrap.io; %2$s - URL to Mailtrap documentation on mailtrap.io. */
				__( '<strong><a href="%1$s" target="_blank" rel="noopener noreferrer">Mailtrap</a></strong> is a modern email platform built to deliver emails fast, and with high inboxing rates. <br>To start sending emails, create an account and set up your domain in Mailtrap.', 'wp-mail-smtp' ),
				[
					'strong' => [],
					'br'     => [],
					'a'      => [
			...
			esc_url( wp_mail_smtp()->get_utm_url( 'https://mailtrap.io/register/signup', [ 'source' => 'wpmailsmtpplugin', 'medium' => 'WordPress', 'content' => isset( $_GET['page'] ) && $_GET['page'] === 'wp-mail-smtp-setup-wizard' ? 'Setup Wizard - Mailer Description' : 'Plugin Settings - Mailer Description' ] ) ),
		);
		...
		if ( empty( $mailer_options['api_key'] ) ) {
			$description .= sprintf(
				...
				esc_url( wp_mail_smtp()->get_utm_url( 'https://mailtrap.io/register/signup', [ 'source' => 'wpmailsmtpplugin', 'medium' => 'WordPress', 'content' => 'Plugin Settings - Mailer Button' ] ) ),
			...
			<div class="wp-mail-smtp-setting-field">
				...
				<p class="desc">
					...
					printf(
						...
						esc_html__( 'Follow this guide to learn how to get %s for setup.', 'wp-mail-smtp' ),
						'<a href="' . esc_url( $get_api_key_url ) . '" target="_blank" rel="noopener noreferrer">' .
						esc_html__( 'API Key', 'wp-mail-smtp' ) .
						'</a>'
					);
					?>
				</p>
